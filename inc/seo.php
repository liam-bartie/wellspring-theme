...
function wellspring_seo_schema( $seo, $title ) {
	...
	if ( is_front_page() ) {
		...
		// The About page is where this person is described, so the node belongs
		// there in full — it is the target every other page's @id points at.
		if ( $post instanceof WP_Post && 'about' === $post->post_name ) {
			$about_person = wellspring_practitioner_person();
			if ( $about_person ) {
				$graph[] = $about_person;
			}
		}
		...
		if ( $post instanceof WP_Post && 'clinic_case' === $post->post_type ) {
			...
			/*
			 * The cases are written by the practitioner from her own clinical
			 * notes, so she is genuinely both author and reviewer. Both are
			 * stated because they answer different questions: who produced this,
			 * and who vouches for it.
			 *
			 * lastReviewed is the modification date rather than a separate
			 * field — it is the only review date the site actually knows, and
			 * inventing a more flattering one would be a lie in markup.
			 */
			$person = wellspring_practitioner_person();

			if ( $person ) {
				$page['author']       = array( '@id' => $person['@id'] );
				$page['reviewedBy']   = array( '@id' => $person['@id'] );
				$page['lastReviewed'] = get_the_modified_date( 'Y-m-d', $post );

				// The node itself travels with the reference, so the page's
				// graph stands alone rather than depending on a crawler having
				// already seen the About page.
				$graph[] = $person;
			}

			$focus = get_the_terms( $post, 'case_focus' );
			if ( $focus && ! is_wp_error( $focus ) ) {
				$page['about'] = array(
